...
const listingTypes = ["Купить", "Продать"];
const categories = ["Бизнес", "Франшиза", "Стартап", "Инвестиции"] as const;
...
export const StepsAddingOffer = () => {
    ...
    const [listingType, setListingType] = useState<null | string>(null);
                        ...
                        {step === 1 && (
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                {categories.map((cat) => (
                                    <div
                                        key={cat}
                                        className={`border rounded-lg px-6 py-4 cursor-pointer text-center ${category === cat ? "border-[#2EAA7B] bg-[#F5FFFA]" : "border-gray-300"
                                            }`}
                                        onClick={() => setCategory(cat)}
                                    >
                                        <Paragraph className="font-semibold">{cat}</Paragraph>
                                    </div>
                                ))}
                            </div>
                        )}
